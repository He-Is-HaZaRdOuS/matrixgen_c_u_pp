...
double lanczos_kernel_cpu(double x, int a = 2) {
    if (x == 0.0) return 1.0;
    if (std::abs(x) > a) return 0.0;
    return a * std::sin(M_PI * x) * std::sin(M_PI * x / a) / (M_PI * M_PI * x * x);
}

CSRMatrix lanczos_cpu_reference(const CSRMatrix& input, double scale_x, double scale_y, double threshold = 1e-6) {
    ...
    for (int i = 0; i < new_rows; i++) {
        for (int j = 0; j < new_cols; j++) {
            ...
            // Lanczos window (radius = 2)
            int start_i = std::max(0, (int)std::floor(orig_i - 2));
            int end_i = std::min(input.rows - 1, (int)std::ceil(orig_i + 2));
            int start_j = std::max(0, (int)std::floor(orig_j - 2));
            int end_j = std::min(input.cols - 1, (int)std::ceil(orig_j + 2));
            ...
            for (int ii = start_i; ii <= end_i; ii++) {
                for (int jj = start_j; jj <= end_j; jj++) {
                    ...
                    if (found) {
                        ...
                        double weight = lanczos_kernel_cpu(dist_i) * lanczos_kernel_cpu(dist_j);
